			  ...
			  switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x03:{//0000 0011
						...
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
						 handle_brake2(robot,-2, 0.5 * robot -> robot_hspeed);
							...
							handle_and_speed(-3, 0.70 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;		
					...
					case 0x01:{//0000 0001					
						...
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
						 handle_brake2(robot,-2, 0.5 * robot -> robot_hspeed);
							...
							handle(&servo,-4);
							set_speed(&motor, 0.70 * robot -> robot_hspeed,0.65 * robot -> robot_hspeed);
					...
					case 0xe0:{//1110 0000					
						...
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
							...
							handle_and_speed(2, 0.68 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = -5; 
							robot -> straight_line_flag = 0;
				   		break;
					...
					case 0xc0:{//1100 0000										
						...
						handle_and_speed(3, 0.65 * robot -> robot_hspeed);
						robot -> curve_flag = 1;
						robot -> sensor_pos = -6; 
						robot -> straight_line_flag = 0;
						break;		
					...
					case 0x80:{//1000 0000					   
						...
						{					
							...
							handle_and_speed(4, 0.65 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = -6; 
							robot -> straight_line_flag = 0;
							break;	 
 		...
 		case CROSS_LINE_TWO:{         //case 22: doi 5ms
			...
			switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x01:{//0000 0001					
						...
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
						 handle_brake2(robot,-3, 0 * robot -> robot_hspeed);
							...
							handle_and_speed(-4,  0 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;			
				...
				if (robot -> cnt1 > 30)
				{
					...
					set_brake(&motor, 0,0);
					handle_and_speed(0, 0.05 * robot -> robot_hspeed);
					robot -> runcase = STOP;
		...
		case TRACE_AFTER_CROSS_LINE:{ //case 23: kiem tra 90 trai hay phai	
			...
			if(  
			  ...
			  || sensor_mask(mask, 0xf8) == 0xf8  //1111 1000
				...
				|| sensor_mask(mask, 0xc0) == 0xc0  //110 0 0000
			  ){			
...
//			robot -> cnt1 = 0;
		...
		case CHANGE_RIGHT_LANE:{      //case 53: chuyen lan phai	
			...
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x00){
				...
				handle_and_speed(RIGHT_CHANGE_LANE_ANGLE, 0.45	* robot -> robot_hspeed);
...
//						}
		...
		case STABLIZE_RIGHT:{         //ON DINH VAO LANE PHAI
			...
			{
				handle_and_speed(8, 0.5*robot -> robot_hspeed);
				HAL_Delay(100);
				robot -> cnt3 = robot ->straight_line_time;
				robot -> runcase  = NORMAL_TRACE;
				break;
...
bool d_check_leftline(){
	if(d_check_crossline() == false) {
	if(sensor_mask(get_sensor_mask(&line_sensor), 0xf8) == 0xf8		//1111 1000
	 ||sensor_mask(get_sensor_mask(&line_sensor), 0xf0) == 0xf0		//1111 0000
	 //||sensor_mask(get_sensor_mask(&line_sensor), 0xe0) == 0xe0		//1110 0000
	 //||sensor_mask(get_sensor_mask(&line_sensor), 0xc0) == 0xc0		//1100 0000
   ||sensor_mask(get_sensor_mask(&line_sensor), 0xfc) == 0xfc		//1111 1100	
	)
	{
