...
void run(Robot *robot) {
	...
	switch(robot -> runcase){
  	...
  	case NORMAL_TRACE:{//case 11: CHAY BINH THUONG	
				if(check_crossline()){robot -> runcase = CROSS_LINE_ONE;break;}
  			if(check_leftline()) {robot -> runcase = LEFT_HALF_LANE_ONE;break;}
				if(check_rightline()){robot -> runcase = RIGHT_HALF_LANE_ONE;break;}		
						
			switch(sensor_mask(get_sensor_mask(&line_sensor), MASK3_3)){
				case 0x00: //000xx000
				{
					/* Center -> straight */
					handle_and_speed(0, robot -> robot_speed);
					break;
				}
				//deviate to the Left//
				case 0x04: //0000 0100
				case 0xc: //0000 1100
				{
					/* Slight amount left of center ->light turn to right */
					handle_and_speed(-2, robot -> robot_speed);
					break;
				}
				...
				case 0x06: //0000 0110
				{
					...
					handle_and_speed(-5, robot -> robot_speed);					
					break;
				}					
				...
				//case 0x03://0000 0011
				{
					...
					handle_and_speed(-8, robot -> robot_speed);
					break;
				}
				case 0x03://0000 0011
				case 0x01://0000 0001
				{
					...
					handle(&servo,-20);
					set_brake(&motor,0,0);
					set_speed(&motor,robot -> robot_speed*1.1, robot -> robot_speed*0.1);
					robot -> cnt1 = 0;
					robot -> runcase = LARGE_RIGHT_TURN; //--> Change to case 12 for configure the large turn RIGHT
					break;
				}
				...
				case 0x80: //1000 0000
				{
					...
					handle(&servo,20);
					set_brake(&motor,0,0);
					set_speed(&motor,robot -> robot_speed*0.1,  robot -> robot_speed*1.1);
					robot -> runcase  = LARGE_LEFT_TURN; //--> Change to case 12 for configure the large turn LEFT
					robot -> cnt1  = 0;
					break;
				}						
				default:
		...
		case LARGE_LEFT_TURN:{//case 12: RE SANG PHAI
				if(check_crossline()){robot -> runcase = CROSS_LINE_ONE;     break;}
  			if(check_leftline()) {robot -> runcase = LEFT_HALF_LANE_ONE; break;}
				if(check_rightline()){robot -> runcase = RIGHT_HALF_LANE_ONE;break;}		
				if(robot -> cnt1 > 5)
				{
					if(sensor_mask(robot -> sensor_mask, MASK3_3) == 0x60){
						robot -> runcase = NORMAL_TRACE;
						robot -> cnt1  = 0;
						}
				}	
		...
		case LARGE_RIGHT_TURN:{//case 13: RE SANG TRAI
				if(check_crossline()){robot -> runcase = CROSS_LINE_ONE;     break;}
  			if(check_leftline()){robot -> runcase = LEFT_HALF_LANE_ONE;  break;}
				if(check_rightline()){robot -> runcase = RIGHT_HALF_LANE_ONE;break;}	
			...
			if(robot -> cnt1 > 5)
						{
							if(sensor_mask(robot -> sensor_mask, MASK3_3) ==  0x06)
							{
								robot -> runcase = NORMAL_TRACE;
								robot -> cnt1 = 0;
							}
		...
		case CROSS_LINE_TWO:{//case 22: doi 5ms
			if (robot -> cnt1 > 5)
			{
				...
				//set_brake(&motor, 1, 1);
				//handle_and_speed(0, robot -> robot_speed*0);
		...
		case TRACE_AFTER_CROSS_LINE:{//case 23: kiem tra 90 trai hay phai
			if(  sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x80  //1000 0000
				|| sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xc0  //1100 0000
				|| sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xe0  //1110 0000
			  || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xf0  //1111 0000
			  || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xfc  //1111 1100
			  || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xfe  //1111 1110
			  ){
				
				// left turn determined -> to the left turn clear processing
				handle_and_speed(0, robot -> robot_speed*0.5);

					robot -> runcase = PREPARE_TO_TURN_LEFT;
				break;
			}
			
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x1     //0000 0001
				 || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x3  //0000 0011
				 || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x7  //0000 0111
				 || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xf  //0000 1111
				 || sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x7f //0111 1111
			  )
			{
			  //left turn determined -> to the left turn clear processing			
					...
					switch(sensor_mask(get_sensor_mask(&line_sensor), MASK3_3)) 
			{
				...
				{
					/* Slight amount left of center ->light turn to right */
					handle_and_speed(-2, robot -> robot_speed*0.5);
					break;
				}
				...
				{
					/* Small amount left of center -> small turn to right */
					handle_and_speed(-5, robot -> robot_speed*0.5);					
					break;
				}					
				...
				case 0x02: //000 0010
				{
					/* Medium amount left of center -> medium turn to right */
//					handle(&servo,-6);
//					set_speed(&motor,robot -> robot_speed*1.1,  robot -> robot_speed);
					handle_and_speed(-6,robot -> robot_speed*0.5);
					break;
				}								
				//deviate to the Right//
				case 0x20: //0010 0000
				...
				case 0x40: //1110 0000
				{
					/* Small amount right of center -> small turn to left */
//					handle(&servo,6);
//					set_speed(&motor,robot -> robot_speed,  robot -> robot_speed*1.1);
					handle_and_speed(6,robot -> robot_speed*0.5);
					break;
				}
				default:
			...
			if (sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x00)  // condition to turn left
				{
					//rgb_setcolor(&rgb, GREEN);
					handle(&servo, 25);
					set_speed(&motor, 0,robot -> robot_speed*0.7);
					robot -> cnt1 =0;	
		...
		case TURN_LEFT_90:{//case 31: doi 80ms
			if (robot -> cnt1 > 50)
			{
		...
		case END_TURN_LEFT_90:{//case 32: ket thuc xoay trai
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK3_3) == 0x06) //0000 0110
			{				
				//rgb_setcolor(&rgb, GREEN);
				handle(&servo, -2);
				robot -> runcase  = NORMAL_TRACE;
		...
		case PREPARE_TO_TURN_RIGHT:{ // CHUAN BI RE PHAI
			if (sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x00)  // condition to turn right
			{
			 // Max angle for turning right ==> Imidiately turn cause => cannot reach 0x60
				handle(&servo, -25);
				set_speed(&motor, robot -> robot_speed*0.7, 0);
				robot -> cnt1  = 0;
		...
		case TURN_RIGHT_90:{//case 41: doi 80ms
			if (robot -> cnt1  > 10)
			{
		...
		case END_TURN_RIGHT_90:{//case 42: ket thuc xoay phai
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK3_3)== 0x60) //01100000
			{
				handle(&servo, 2);
				robot -> runcase = NORMAL_TRACE;
			}
		...
		case RIGHT_HALF_LANE_ONE:{// case 51: cfhuyen lan phai
			//if(check_crossline()){robot -> runcase = CROSS_LINE_ONE;break;}
			robot -> runcase  = CHANGE_RIGHT_LANE;
			break;
		}
		
		case RIGHT_HALF_LANE_TWO:{// case 52: doi 50ms
			if (robot -> cnt1   > 50)
			{
				set_brake(&motor,0,0);
				robot -> runcase  = CHANGE_RIGHT_LANE;		
		...
		case CHANGE_RIGHT_LANE:{//case 53: chuyen lan phai
			...
			{
				handle_and_speed(-11, 0.8* robot -> robot_speed);
			...
			switch(sensor_mask(get_sensor_mask(&line_sensor), MASK3_3))
			{
				...
				case 0x3c:
				{
					// 001xx100 ->> This case hardly happens
					/* Center -> straight */
					handle_and_speed(0, robot -> robot_speed);
					break;
				}
				...
				case 0x04:
				{
					...
					handle_and_speed(-2, robot -> robot_speed*0.8);
					break;
				}
				...
				case 0x06:
				{
					//0110
					/* Small amount left of center -> small turn to right */
					handle_and_speed(-5, robot -> robot_speed*0.6);   //0.8 * speed_run
					break;
				}
				...
				{
					/* Medium amount left of center -> medium turn to right */
					handle_and_speed(-7, robot -> robot_speed*0.6); //0.6 * speed_run
					break;
				}
				...
				{
					/* Small amount right of center -> small turn to left */
					handle_and_speed(5, robot -> robot_speed*0.6);//convert_speed(0.8 * speed_run)
					break;
				}
				...
				case 0x40: //0100 0000
				{
					...
					handle_and_speed(7, robot -> robot_speed * 0.6); //convert_speed( 0.6 * speed_run)
					break;
				}
				case 0xe7:
				{
		...
		case STABLIZE_RIGHT:{ //ON DINH VAO LANE PHAI
			if(sensor_mask(get_sensor_mask(&line_sensor), 0x01) != 0)
			{
				handle_and_speed(0, 0.8*robot -> robot_speed);
		...
		case END_CHANGE_RIGHT_LANE:{// case 54: end chuyen lan phai
			if (sensor_mask(get_sensor_mask(&line_sensor), MASK4_0) != 0)
			{
				//robot_rgb.setColor(RGB::OFF);
				robot -> runcase  = NORMAL_TRACE;	
			}
			break;
		}
		...
		case LEFT_HALF_LANE_TWO:{// case 62: doi 50ms
			...
			{
//				robot_motors.setBrake(0,0);
//				robot_motors.setSpeed(convert_speed(0.4*speed_run),convert_speed(0.4*speed_run));
				robot -> runcase  = CHANGE_LEFT_LANE;
			}
			break;
		}
		
		case CHANGE_LEFT_LANE:{//case 63: chuyen lan trai
		...
		case STABLIZE_LEFT:{ //ON DINH VAO LANE TRAI
			if(sensor_mask(get_sensor_mask(&line_sensor), 0x80) != 0)
			{
				handle_and_speed(0, 0.8 * robot-> robot_speed);
		...
		case END_CHANGE_LEFT_LANE:{//case 64: end chuyen lan phai
...
bool check_crossline(){// HAM KIEM TRA GOC 90
	rgb_setcolor(&rgb, OFF);
	bool ret = false; 
	if(
			sensor_mask(get_sensor_mask(&line_sensor), 0xe7)   == 0xe7    //1110 0111
	  ||  sensor_mask(get_sensor_mask(&line_sensor), 0xc3)   == 0xc3    //1100 0011
		||  sensor_mask(get_sensor_mask(&line_sensor), 0xff)   == 0xff    //1111 1111
	  ||  sensor_mask(get_sensor_mask(&line_sensor), 0x81)   == 0x81    //1000 0001
	)
	{
		ret = true;
...
bool check_rightline(){// HAM KIEM TRA CHUYEN LAN PHAI
		rgb_setcolor(&rgb, OFF);
		bool ret = false;
		if (check_crossline() == false) {
	  if(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x1f //0001 1111
	   ||sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x0f  //0000 1111 - 0D
		||sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x3f  //0000 1111 - 0D
		||sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x7f  //0111 1111 - 0D
		)
		{
...
bool check_leftline(){//  HAM KIEM TRA CHUYEN LAN TRAI
	bool ret = false;
	if (check_crossline() == false) {
  	...
  	case NORMAL_TRACE:{           //case 11: CHAY BINH THUONG	
			  ...
			  switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x0e:{//0000 1110
					 if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-4, 0.7 * robot -> robot_hspeed);
							break;
						}
						else if( robot -> brake_time  > robot -> min_brake_time)
						{						
							handle_brake2(robot,-4, 0.8 * robot -> robot_hspeed);
						}
					...
					case 0x06:{//0000 0110
						...
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-5, 0.7 * robot -> robot_hspeed);
							break;
						}
						else if( robot -> brake_time  > robot -> min_brake_time)
						{							
							handle_brake2(robot, -5, 0.7 * robot -> robot_hspeed);
						}
					...
					case 0x07:{//0000 0111
						...
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-6,0.7 * robot -> robot_hspeed);
							break;
						}
						else if( robot -> brake_time  > robot -> min_brake_time)
						{	
							handle_brake2(robot,-6,0.5 * robot -> robot_hspeed);
						}
					...
					case 0x03:{//0000 0011
						...
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-7, 0.75 * robot -> robot_hspeed);
							break;
						}
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
						 handle_brake2(robot,-7, 0.5 * robot -> robot_hspeed);
						}
					...
					case 0x01:{//0000 0001					
						...
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-8, 0.75 * robot -> robot_hspeed);
							break;
						}
						else if( robot -> brake_time  > robot -> min_brake_time)
						{
						 handle_brake2(robot,-8, 0.5 * robot -> robot_hspeed);
						}
							handle_and_speed(-5,  0.65 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
 					...
 					case 0x00:{//0000 0000										
						...
						{
							handle_and_speed(12, 0 * robot -> robot_hspeed);												
							break;
						}
 		...
 		case CROSS_LINE_TWO:{         //case 22: doi 5ms
			...
			switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x08:{//0000 1000					
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-22, 0* robot -> robot_hspeed);
							break;
						}
					...
					case 0x0e:{//0000 1110
					 if (robot -> sensor_pos < -3)
						...
						{						
							handle_brake2(robot,-5, 0 * robot -> robot_hspeed);
						}
							handle_and_speed(-5, 0* robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 3;
							robot -> straight_line_flag = 0;			
							break;
					...
					case 0x06:{//0000 0110
						...
						{							
							handle_brake2(robot, -7, 0 * robot -> robot_hspeed);
						}
							handle_and_speed(-7,0 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 4; 
							robot -> straight_line_flag = 0;
							break;	
					...
					case 0x00:{//0000 0000										
						if (robot -> sensor_pos < -4)
						{
							handle_and_speed(25, 0 * robot -> robot_hspeed);												
							break;
						}
						else if (robot -> sensor_pos > 4)
						{
		...
		case TRACE_AFTER_CROSS_LINE:{ //case 23: kiem tra 90 trai hay phai		
			if(  
			  ...
			  || sensor_mask(get_sensor_mask(&line_sensor), 0xf8) == 0xf8  //11111000
				|| sensor_mask(get_sensor_mask(&line_sensor), 0xfc) == 0xfc  //11111100
			  ){			
					//robot -> runcase = PREPARE_TO_TURN_LEFT;
						handle(&servo, 25);
						set_brake(&motor, 0,0);
			...
			if(   
				 ...
				 ||sensor_mask(get_sensor_mask(&line_sensor), 0x1f) == 0x1f //00011111
				 ||sensor_mask(get_sensor_mask(&line_sensor), 0x3f) == 0x3f //00111111
			  ){
					handle(&servo, -25 );
					set_brake(&motor, 0,0);
					set_speed(&motor, 50 , 0);
					robot -> cnt1  = 0;
					robot -> runcase  = TURN_RIGHT_90;
				//robot -> runcase = PREPARE_TO_TURN_RIGHT;
				break;
		  ...
		  // Type 1 when approaching the strainght line before turning.		
			switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x08:{//0000 1000					
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-22, CROSS_LINE_FACTOR * robot -> robot_hspeed);
							break;
						}
					...
					case 0x06:{//0000 0110
						...
						if (robot -> sensor_pos < -3)
						{
							...
							handle_and_speed(-7, CROSS_LINE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 4; 
							robot -> straight_line_flag = 0;
							break;	
					...
					case 0x07:{//0000 0111
						...
						if (robot -> sensor_pos < -3)
						{
							...
							handle_and_speed(-14, CROSS_LINE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 5; 
							robot -> straight_line_flag = 0;
							break;	
					...
					case 0x03:{//0000 0011
						...
						if (robot -> sensor_pos < -3)
						{
							...
							handle_and_speed(-17, CROSS_LINE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;		
					...
					case 0x01:{//0000 0001					
						...
						if (robot -> sensor_pos < -3)
						{
							...
							handle_and_speed(-18,  CROSS_LINE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;			
					...
					case 0x00:{//0000 0000										
						if (robot -> sensor_pos < -4)
						{
							handle_and_speed(25, CROSS_LINE_FACTOR* robot -> robot_hspeed);												
							break;
						}
						else if (robot -> sensor_pos > 4)
						{
					...
					case 0xc0:{//1100 0000										
						if (robot -> sensor_pos > 3)
						{
							handle_and_speed(22, 0.7 * robot -> robot_hspeed);
							break;
						}
						handle_and_speed(17, CROSS_LINE_FACTOR * robot -> robot_hspeed);
						robot -> curve_flag = 1;
		...
		case END_TURN_LEFT_90:{       //case 32: ket thuc xoay trai
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x60 //0011 1100
				||  sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0xe0     //0000 1110
				||sensor_mask(get_sensor_mask(&line_sensor), 0x3c) == 0x3c)
			{	
				robot -> runcase  = NORMAL_TRACE;
...
//				robot -> runcase = NORMAL_TRACE;
//			}
			if(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x06 //0011 1100
			|| sensor_mask(get_sensor_mask(&line_sensor), 0xc3)    == 0xc3
			|| sensor_mask(get_sensor_mask(&line_sensor), MASK4_4) == 0x60 )
			{	
				robot -> runcase  = NORMAL_TRACE;
				robot -> cnt3 = robot ->straight_line_time;
				break;
...
//				case 0x0c://0000 1100
//				case 0x04://0000 1000
//				handle_and_speed (-7, CHANGE_LANE_FACTOR * robot -> robot_hspeed);
//				case 0xe7:
//				case 0xff:
//				{
//					if(d_check_crossline()){robot -> runcase = CROSS_LINE_ONE;break;}
...
//			}			
       switch(sensor_mask(get_sensor_mask(&line_sensor), MASK4_4)){
					...
					case 0x08:{//0000 1000					
						if (robot -> sensor_pos < -3)
						{
							handle_and_speed(-22, CHANGE_LANE_FACTOR * robot -> robot_hspeed);
							break;
						}
						handle_and_speed(-2, CHANGE_LANE_FACTOR * robot -> robot_hspeed);
						robot -> curve_flag = 0;
...
//						}
							handle_and_speed(-7, 0.7 * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 4; 
							robot -> straight_line_flag = 0;
							break;	
...
//						}
							handle_and_speed(-14, CHANGE_LANE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 5; 
							robot -> straight_line_flag = 0;
							break;	
...
//						}
							handle_and_speed(-17, CHANGE_LANE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;		
...
//						}
							handle_and_speed(-18,  CHANGE_LANE_FACTOR * robot -> robot_hspeed);
							robot -> curve_flag = 1;
							robot -> sensor_pos = 6; 
							robot -> straight_line_flag = 0;
							break;			
					...
					case 0x00:{//0000 0000										
						if (robot -> sensor_pos < -4)
						{
							handle_and_speed(25, CHANGE_LANE_FACTOR * robot -> robot_hspeed);												
							break;
						}
						else if (robot -> sensor_pos > 4)
						{
...
//						}
		...
		case STABLIZE_RIGHT:{         //ON DINH VAO LANE PHAI
			if(sensor_mask(get_sensor_mask(&line_sensor), 0x07) != 0
				||sensor_mask(get_sensor_mask(&line_sensor), MASK0_4) != 0 )
			{
		...
		case END_CHANGE_RIGHT_LANE:{  //case 54: end chuyen lan phai
			if (sensor_mask(get_sensor_mask(&line_sensor), MASK4_0)  != 0x00
			 || sensor_mask(get_sensor_mask(&line_sensor), 0xc3)     != 0xc3
			 || sensor_mask(get_sensor_mask(&line_sensor), 0x60)     == 0x60
			 || sensor_mask(get_sensor_mask(&line_sensor), 0x06)     == 0x06)
			{
				robot -> cnt3 = robot ->straight_line_time;
				set_brake(&motor, 1,1);
				set_speed(&motor,0,0);
...
//					||sensor_mask(get_sensor_mask(&line_sensor), MASK4_0) != 0
		...
		case END_CHANGE_LEFT_LANE:{   //case 64: end chuyen lan trai			
			if (sensor_mask(get_sensor_mask(&line_sensor), MASK0_4)  != 0
			 || sensor_mask(get_sensor_mask(&line_sensor), 0xc3)     != 0xc3
			 || sensor_mask(get_sensor_mask(&line_sensor), 0x60)     == 0x60
			 || sensor_mask(get_sensor_mask(&line_sensor), 0x06)     == 0x06)
			{
